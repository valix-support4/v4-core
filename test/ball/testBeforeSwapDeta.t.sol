...
contract CustomAccountingTest is Test, Deployers, GasSnapshot {
    ...
    function test_fuzz_swap_beforeSwap_returnsDeltaSpecifiedBall(
        
    ) public {
        // ------------------------ SETUP ------------------------
        int128 hookDeltaSpecified = 0;
        int256 amountSpecified = -100e6;
    ...
    function test_fuzz_swap_beforeSwap_returnsDeltaSpecifiedBall2(
        
    ) public {
        // ------------------------ SETUP ------------------------
        // case 1
        int128 hookDeltaSpecified = -1e6;
        int256 amountSpecified = -100e6;
        bool zeroForOne = true;
        Currency specifiedCurrency;
        bool isExactIn;
        ...
        specifiedCurrency = (isExactIn == zeroForOne) ? key.currency0 : key.currency1;
        // case 1
        // specifiedCurrency = (true == true) ? key.currency0 : key.currency1;
        // specifiedCurrency = key.currency0;
        ...
        if (amountSpecified == 0) {
            ...
            // case 1
            // isExactIn && (hookDeltaSpecified > -amountSpecified) ?
            // true && (-1e6 > 100e6) ?
            // true & false
            // else if(false)
            ...
            // case 1
            // !isExactIn && (amountSpecified < -hookDeltaSpecified) ?
            // false && NA ?
            // else if(false)
            vm.expectRevert(Hooks.HookDeltaExceedsSwapAmount.selector);
            swapRouter.swap(key, params, testSettings, ZERO_BYTES);
            ...
            uint256 balanceThisBefore = specifiedCurrency.balanceOf(address(this));
            console.log("file: testBeforeSwapDeta.t.sol:365 ~ balanceThisBefore:", balanceThisBefore);
            uint256 balanceHookBefore = specifiedCurrency.balanceOf(hook);
            console.log("file: testBeforeSwapDeta.t.sol:367 ~ balanceHookBefore:", balanceHookBefore);
            uint256 balanceManagerBefore = specifiedCurrency.balanceOf(address(manager));
            console.log("file: testBeforeSwapDeta.t.sol:369 ~ balanceManagerBefore:", balanceManagerBefore);
            // case 1
            // balanceThisBefore = 57896044618658097711785492504343953926294709965899343556265405433049275589187
            // balanceHookBefore = 340282366920938463463374607431768211455
            // balanceManagerBefore = 11963475521019326

            // case 1
            // จะได้ params การ swap เป็น
            ...
            // });
            BalanceDelta delta = swapRouter.swap(key, params, testSettings, ZERO_BYTES);
            // เรียก PoolSwapTest.swap
            
            console.log("delta.amount0() in test file ");
            console.logInt(delta.amount0());
            console.log("delta.amount1() in test file ");
            console.logInt(delta.amount1());
            int128 deltaSpecified = (zeroForOne == isExactIn) ? delta.amount0() : delta.amount1();
